...
def cy_srad(float [:, :] array,float step=0.05, int iter=0):
    ...
    for i in range(1, ny-1):
        for j in range(1, nx-1):
                    d = (ci[i+1, j]*(array[i+1, j]-array[i, j])
                        + ci[i, j]*(array[i-1, j]-array[i, j])
                        + ci[i, j+1]*(array[i, j+1]-array[i, j])
                        + ci[i, j]*(array[i, j-1]-array[i, j]))
    ...
    for i in [0, ny-1]:
        for j in [0, nx-1]:
            ...
            out[i, j] = array[ i, j] + step/4.0*(cip*(aip-array[i, j])
                + ci[i, j]*(aim-array[ i, j]) + cjp*(ajp-array[ i, j])
...
@cython.boundscheck(False)
@cython.wraparound(False)
def cy_csrad(float [:, :] array,float step=0.05, int iter=0):
    cdef float p1, p2, p3, p4, sip, sim, sjp, sjm
    cdef float aim, aip, ajm, ajp
    cdef int i, j
    cdef float d2i, qp1, qp2, q2
